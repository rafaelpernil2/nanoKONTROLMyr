...
class SliderSection:
    ...
    def build_midi_map(self, script_handle, midi_map_handle):
        ...
        for channel in range(16):
            Live.MidiMap.map_midi_cc_with_feedback_map(midi_map_handle, self._SliderSection__parent.song().master_track.mixer_device.volume, channel, NANOKONTROL_SLI9, Live.MidiMap.MapMode.absolute, feedback_rule, 1)

        for channel in range(4):
            Live.MidiMap.forward_midi_cc(script_handle, midi_map_handle, channel, NANOKONTROL_BUT9)
            for slider in range(8):
                ...
                if (len(self._SliderSection__parent.song().tracks) > track_index):
                    ...
                    feedback_rule.cc_value_map = tuple()
                    feedback_rule.delay_in_ms = -1.0
                    Live.MidiMap.map_midi_cc_with_feedback_map(midi_map_handle, self._SliderSection__parent.song().tracks[track_index].mixer_device.volume, channel, NANOKONTROL_SLIDERS[slider], Live.MidiMap.MapMode.absolute, feedback_rule, 1)
                    Live.MidiMap.forward_midi_cc(script_handle, midi_map_handle, channel, NANOKONTROL_BUTTONS[slider])
                else:
                    break
